...
def compute_relative():
    analytical = pd.read_csv("dfds.dat", delim_whitespace=True, header=None)
    analytical_len = len(analytical)

    fdm = pd.read_csv("fdm.dat", delim_whitespace=True, header=None)
    fdm_len = len(fdm)
...
def plot_sensitivity():
    args =  '''
                ...
                set key box;
                plot 
                'dfds.dat'      with lines      linewidth 2.5                      title "Adjoint Method",
                'fdm.dat'                  pt 13 ps 1.5  title "FDM" ;
                pause -1;
            '''
    call([ "gnuplot", "-e", args])
